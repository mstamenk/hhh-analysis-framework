...
hist_properties = {'JetHT' : [ROOT.kBlack, 0.8, 0, 'Data', True] ,
                  ...
                  'Wto2Q-3Jets'   : [ROOT.kCyan, 0, 0, 'V+jets', False],
                   ...
                   'HHHTo4B2Tau_c3_0_d4_0_TuneCP5_13TeV-amcatnlo-pythia8' : [ROOT.kRed, 0,3, 'SM HHH4b2tau', True],
                   'GluGluToHHH' : [ROOT.kRed, 0,3, 'SM HHH', True],
                   'GluGluToHHTo4B_cHHH1' : [ROOT.kViolet + 2, 0,3, 'SM HH', True],
                   'GluGluToHHTo4B_cHHH1_TuneCP5_PSWeights_13TeV-powheg-pythia8' : [ROOT.kViolet + 2, 0,3, 'SM HH', True],
...
getmax_v34 = '''
int get_max_prob_v34(float ProbHHH, float ProbQCD, float ProbTT, float ProbHH4b){
    std::vector<float> probs;
    probs.push_back(ProbHHH); // 1
    probs.push_back(ProbQCD); // 2
    probs.push_back(ProbTT); // 3
...
def init_get_max_prob_v34():
    ROOT.gInterpreter.Declare(getmax_v34)



gethandcat = '''
int get_hand_cat(int nFatJetsPassed, int nJetsPassed, float hhh_mass) {
...
def add_correction_factor(df, nbins=500, ht_min=0.0, ht_max=5000.0, 
                          hist_name="h_ratio", func_name="correctionFunc"):
    """
    Given an RDataFrame df, compute the correction factor vs. ht and
    return a new RDataFrame with an extra column 'correctionFactor'.

    Parameters:
    -----------
    df        : ROOT.RDataFrame
        Input frame containing at least the columns 'ht', 'eventWeight',
        'flavTagWeight', and 'fatJetFlavTagWeight'.
    nbins     : int
        Number of bins for the ht histograms.
    ht_min    : float
        Lower bound of the ht histograms.
    ht_max    : float
        Upper bound of the ht histograms.
    hist_name : str
        Name for the ratio histogram.
    func_name : str
        Name for the TF1 that wraps the ratio.

    Returns:
    --------
    df2       : ROOT.RDataFrame
        A new RDataFrame with an additional column 'correctionFactor'.
    h_ratio   : ROOT.TH1D
        The histogram holding the ratio vs. ht.
    """

    # 1) Produce the two hists (these create "action" nodes but do not
    #    actually run until you ask for the histogram object)
    h_nom = df.Histo1D(("ht_num", "ht_num", nbins, ht_min, ht_max),"ht","eventWeight").GetValue()


    df = df.Define('eventWeightNoFTAG',"eventWeight/(flavTagWeight*fatJetFlavTagWeight)")
    h_alt = df.Histo1D(("ht_num_den", "ht_num_den", nbins, ht_min, ht_max),"ht",'eventWeightNoFTAG').GetValue()

    # 2) Build the ratio histogram
    h_ratio = h_alt.Clone(hist_name)
    h_ratio.Divide(h_nom)

    # 3) Register the histogram as a global object so that our TF1 can see it
    ROOT.gROOT.cd()  # ensure gDirectory is writable
    h_ratio.SetDirectory(ROOT.gROOT)
    ROOT.gROOT.GetListOfGlobals().Add(h_ratio)

    # 4) Create a TF1 that looks up the bin content of h_ratio
    #    Note: the tiny C++ snippet is wrapped in a DoubleFunction

    hist_test = f'auto h_ratio = (TH1*)gROOT->FindObject("{hist_name}");'
    ROOT.gInterpreter.Declare(hist_test)


    # 5) Define the new column in the RDataFrame
    df = df.Define("htCorrection", f"get_hist(h_ratio,ht)")
...
passPtMin = '''
 int passPtMin(float j1, float j2, float j3, float j4, float j5,
         float j6, float j7, float j8, float j9, float j10) {
        // pack into fixed array
        std::array<float,10> all{{j1,j2,j3,j4,j5,j6,j7,j8,j9,j10}};
        std::vector<float> sel;
        sel.reserve(10);
        for (auto pt : all) {
          if (pt > 20.0) sel.push_back(pt);
        }
        if (sel.empty()) return 1;
        std::sort(sel.begin(), sel.end());
        return int(sel.front() > 25.0);
      }
'''

ROOT.gInterpreter.Declare(passPtMin)

def addMinPtCut(df):
    return df.Define("passMinPt", f"passPtMin(jet1Pt,jet2Pt,jet3Pt,jet4Pt,jet5Pt,jet6Pt,jet7Pt,jet8Pt,jet9Pt,jet10Pt)")


passfjPtMin = '''
 int passfjPtMin(float j1, float j2, float j3) {
        // pack into fixed array
        std::array<float,3> all{{j1,j2,j3}};
        std::vector<float> sel;
        sel.reserve(3);
        for (auto pt : all) {
          if (pt > 200.0) sel.push_back(pt);
        }
        if (sel.empty()) return 0;
        std::sort(sel.begin(), sel.end());
        return int(sel.front() > 300.0);
      }
'''

ROOT.gInterpreter.Declare(passfjPtMin)

def addFJMinPtCut(df):
    return df.Define("passFJMinPt", f"passfjPtMin(fatJet1Pt,fatJet2Pt,fatJet3Pt)")