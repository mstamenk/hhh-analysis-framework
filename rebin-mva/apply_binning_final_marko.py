...
for cat in ['%s_0bh3h_inclusive','%s_3bh0h_inclusive','%s_2bh1h_inclusive','%s_1bh2h_inclusive','%s_2bh0h_inclusive','%s_1bh1h_inclusive','%s_0bh2h_inclusive','%s_1bh0h_inclusive','%s_0bh1h_inclusive','%s_0bh0h_inclusive','%s_1bh0h_inclusive']:# variables:
# for cat in ['%s_1bh0h_inclusive','%s_0bh1h_inclusive']:# variables:
    cat = cat%prob
    print(cat)
    #print(binnings[cat])
    target = '%s%s/histograms'%(cat,option)
    output_path = '/eos/user/x/xgeng/workspace/HHH/CMSSW_12_5_2/src/hhh-analysis-framework/output/v34_fix_ak4_ak8_JME/%s' % path_year

    if not os.path.isdir(output_path):
        os.makedirs(output_path)


    if not os.path.isdir(output_path + '/' + target):
    ...
    print("year is %s, binning is %s"%(year, binning))
    # break

    #var = "ProbMultiH" #variables[cat]
    outfile = ROOT.TFile(output_path +'/' + target + '/' + 'histograms_%s.root'%var,'recreate')
    ...
    for s in samples:
        ...
        if s in  samples_with_jme:
            for jme in ["JES", "JER", "JMR"]:
                try:
                    h_up, h_down = apply_jme_variations(h_mva, s,cat,args.path_year,f"{jme}UP",binning,verbose = True)
                    if h_up: h_up.Write()
    ...
    for sam in samples:
        ...
        for syst in systematics:
            ...
            if 'cHHH' in s:
                h_mva.SetTitle(s.replace('_TuneCP5_PSWeights_13TeV-powheg-pythia8',''))
                h_mva.SetName(s.replace('_TuneCP5_PSWeights_13TeV-powheg-pythia8',''))
            outfile.cd()
            h_mva.Write()
