...
from ht_trigger_syst import load_ht_histogram, declare_ht_syst_weights
...
ht_histo = load_ht_histogram(args.year, base_dir="trigger-correction")
ht_weights = declare_ht_syst_weights(ht_histo, ht_col="ht", weight_col="totalWeight")
# ht_weights == {"HTUp":"totalWeight*getHTCorr(ht)", "HTDown":"totalWeight/getHTCorr(ht)"}
...
path = '/isilon/data/users/mstamenk/eos-triple-h/%s/mva-inputs-%s-categorisation-spanet-boosted-classification/'%(version,args.year)
...
#for cat in ['%s_2bh0h_inclusive','%s_1bh1h_inclusive','%s_0bh2h_inclusive','%s_1bh0h_inclusive','%s_0bh1h_inclusive','%s_0bh0h_inclusive','%s_2Higgs_inclusive','%s_1Higgs_inclusive','%s_3Higgs_inclusive']:# variables:
        ...
        if args.doSyst:
            ...
            if s in  samples_with_jme:
                for jme in ["JES", "JER", "JMR"]:
                    try:
                        h_up, h_down = apply_jme_variations(h_mva, s,cat,args.year,f"{jme}UP",binning,verbose = False)
                        if h_up: h_up.Write()
    ...
    for sam in samples:
        ...
        for syst in systematics:
            ...
            elif syst in ('HTUp',"HTDown"):
                print(syst)
                weight = ht_weights[syst]
