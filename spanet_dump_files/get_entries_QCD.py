import os
import ROOT

def get_file_size_and_entries(file_path):
    """获取ROOT文件大小和entries数量"""
    # 获取文件大小，并转换为GB
    file_size = os.path.getsize(file_path) / (1024 ** 3)  # 转换为GB
    
    # 使用ROOT读取文件并获取entries数目
    try:
        file = ROOT.TFile(file_path)
        tree = file.Get("Events")  # 假设树的名字是'Events'
        entries = tree.GetEntries() if tree else 0
        return file_size, entries
    except Exception as e:
        print(f"Error reading {file_path}: {e}")
        return 0, 0

def scan_directory(directory_path):
    """扫描目录，输出符合条件的QCD_HT开头的ROOT文件大小和entries，并计算总和"""
    total_size_gb = 0
    total_entries = 0
    for root, dirs, files in os.walk(directory_path):
        for file in files:
            if file.endswith(".root") and file.startswith("QCD_HT") and "QCD_datadriven" not in file:
                file_path = os.path.join(root, file)
                file_size, entries = get_file_size_and_entries(file_path)
                total_size_gb += file_size
                total_entries += entries
                print(f"File: {file}, Size: {file_size:.2f} GB, Entries: {entries}")
    
    # 打印总大小和总entries
    print(f"Total size of QCD_HT files: {total_size_gb:.2f} GB")
    print(f"Total entries in QCD_HT files: {total_entries}")

# 使用你的路径替换下面的示例路径
directory_path = "/eos/cms/store/group/phys_higgs/cmshhh/v34_ak8_option4_2018/mva-inputs-2018/inclusive-weights"
scan_directory(directory_path)
