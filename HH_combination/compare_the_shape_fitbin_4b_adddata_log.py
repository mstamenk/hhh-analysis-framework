...
parser.add_option('--log', action='store_true', dest='log_scale', default=False, help="Enable log scale on y-axis")
parser.add_option('--rebin', action='store_true', dest='rebin', default=False, help="Enable log scale on y-axis")
...
cat_list = ["GGFcateg1_20172018"]
type_list = ["alt","alt2"]
type_list = ["alt"]
for type in type_list:
    path_fit = "/eos/home-x/xgeng/workspace/HHH/CMSSW_12_5_2/src/hhh-analysis-framework/HH_combination/fitdiagnose/bbbb"
    if rebin:
        path_plots = "/eos/home-x/xgeng/workspace/HHH/CMSSW_12_5_2/src/hhh-analysis-framework/HH_combination/plots/bbbb/plots_%s_rebin_data"%(type)
    else:
        path_plots = "/eos/home-x/xgeng/workspace/HHH/CMSSW_12_5_2/src/hhh-analysis-framework/HH_combination/plots/bbbb/plots_%s_data"%(type)
    if type == "alt2":
        if rebin:
            path_r = "/eos/home-x/xgeng/workspace/HHH/CMSSW_12_5_2/src/hhh-analysis-framework/HH_combination/bbbb/new_hist_alt2_rebin"
        else:
            path_r = "/eos/home-x/xgeng/workspace/HHH/CMSSW_12_5_2/src/hhh-analysis-framework/HH_combination/bbbb/new_hist_alt2"
    else:
        if rebin:
            path_r = "/eos/home-x/xgeng/workspace/HHH/CMSSW_12_5_2/src/hhh-analysis-framework/HH_combination/bbbb/new_hist_alt_rebin"
        else:
            path_r = "/eos/home-x/xgeng/workspace/HHH/CMSSW_12_5_2/src/hhh-analysis-framework/HH_combination/bbbb/new_hist_alt"
    ...
    for cat in cat_list:
        ...
        hist_data_o = file_r.Get("data_obs")
        ...
        hist_data = ROOT.TH1F("hist_data", "Histogram with Non-Uniform Bins", num_bins, array('d', bin_edges))
        hist_data.SetTitle(fit_title)
        hist_data.GetXaxis().SetTitle(fit_xaixs_title)
        ...
        for i in range(1, num_bins + 1):
            ...
            content_data = hist_data_o.GetBinContent(i)
            error_data = hist_data_o.GetBinError(i)
            hist_data.SetBinContent(i, content_data)
            hist_data.SetBinError(i, error_data)

        if log_scale:
            offset  = 20 
            for hist in [hist_target, hist_fit, hist_data]:
                for bin in range(1, hist.GetNbinsX() + 1):
                    current_content = hist.GetBinContent(bin)
                    print(f"Before offset: {current_content}")
                    hist.SetBinContent(bin, current_content + offset)
                    print(f"After offset: {hist.GetBinContent(bin)}") 

        if not hist_target.GetSumw2N():
            hist_target.Sumw2()
        if not hist_fit.GetSumw2N():
            hist_fit.Sumw2()
        if not hist_data.GetSumw2N():
            hist_data.Sumw2()
        ...
        hist_data.SetMarkerStyle(20)  # 设置数据点的标记样式（比如圆圈）
        hist_data.SetMarkerColor(ROOT.kRed)  # 设置数据点颜色为红色
        hist_data.SetMarkerSize(1.2) 
        all_max = max(hist_target.GetMaximum(), hist_fit.GetMaximum(), hist_data.GetMaximum())
        ...
        if log_scale:
            line_zero_pad1 = ROOT.TLine(hist_target.GetXaxis().GetXmin(), offset, hist_target.GetXaxis().GetXmax(), offset)
        else:
        ...
        if log_scale:
            c1.SaveAs(f"{path_plots}/comparison_{cat}_log.pdf")
        else:
