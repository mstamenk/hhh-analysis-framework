...
for j in ['jet1','jet2','jet3','jet4','jet5','jet6','jet7','jet8','jet9','jet10']:
    higgsmatched.append('int(%sHiggsMatched && %sFatJetMatched == 0)'%(j,j))

higgsMatchVar = '+'.join(higgsmatched)

df = df.Define('nAK4matched2', higgsMatchVar)
...
matrix = df.Histo2D(('cm','cm',binx,xmin,xmax,biny,ymin,ymax), 'categorisation','nAK4matched2','eventWeight')
...
c.Print('no_overlap_matrix_%s_%s_%s_%s_categorisation.png'%(typename,year,sample,maxprob))
...
c2.Print('no_overlap_normalised_%s_%s_%s_%s_categorisation.png'%(typename,year,sample,maxprob))
