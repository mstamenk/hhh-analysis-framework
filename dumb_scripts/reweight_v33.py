...
kappa_list = {'c3_m14_d4_m101' : {'k3': -13, 'k4': -100},
              'c3_m11_d4_99'   : {'k3': -10, 'k4': 100},
              'c3_m5_d4_m21'   : {'k3': -4, 'k4': -20},
              'c3_m6_d4_29'    : {'k3': -5, 'k4': 30},
              'c3_2_d4_m61'     : {'k3': 3, 'k4': -60},
              'c3_7_d4_49'     : {'k3': 8, 'k4': 50},
              'c3_7_d4_m21'     : {'k3': 8, 'k4': -20},
              'c3_15_d4_m81'     : {'k3': 16, 'k4': -80},
...
for cat in cat_list:
    ...
    for var in  var_list:
        if use_scale:
            # file = ROOT.TFile("%s/histograms_%s_scale.root"%(path_for_histograms,var),"READ")
            # output_file = ROOT.TFile("%s/histograms_%s_scale_reweight.root"%(path_for_histograms,var), "RECREATE")
            file = ROOT.TFile("%s/histograms_%s_scale2.root"%(path_for_histograms,var),"READ")
            output_file = ROOT.TFile("%s/histograms_%s_scale2_reweight.root"%(path_for_histograms,var), "RECREATE")
        else:
        ...
        if hist_QCD:
            hist_QCD_tmp = hist_QCD.Clone()
            hist_QCD_tmp.SetTitle('QCD')
                    # print(weight)
            hist_QCD_tmp.SetName('QCD')
            hist_QCD_tmp.Write()
            print("Standard Model histogram has been written.")
        else:
