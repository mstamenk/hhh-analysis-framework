...
ROOT::RDF::RNode AddArray(ROOT::RDF::RNode df, ROOT::RVec<double> &v, const std::string &name) {
    return df.Define(name, [&](unsigned int e) { return v[e]; }, {"counter_v34"});
}
...
if '2022' in args.year:
    ...
    session = onnxruntime.InferenceSession("/users/mstamenk/hhh-analysis-framework/spanet-inference/spanet_v34_run2_3Higgs_v0.onnx",sess_options, providers=['CPUExecutionProvider'])
...
df = df.Define('counter_v34','counter_v34++')
...
#index_arrays = []
#index_vars = ['IndexMaxCat']
#column = [el for el in ht_vars]
#np_dict = df.AsNumpy(column)
#np_arr = np.vstack([np_dict[col] for col in column]).T.astype(np.float32)
#index_arrays.append(np_arr)
...
for i in range(len(output_values[0])):
    ...
    prob_hh4b.append(float(output_values[12][i][4]))
...
df = ROOT.AddArray(ROOT.RDF.AsRNode(df), arr_h1_mass, "h1_spanet_boosted_mass_v34bis")
df = ROOT.AddArray(ROOT.RDF.AsRNode(df), arr_h1_pt, "h1_spanet_boosted_pt_v34bis")
df = ROOT.AddArray(ROOT.RDF.AsRNode(df), arr_h1_eta, "h1_spanet_boosted_eta_v34bis")
df = ROOT.AddArray(ROOT.RDF.AsRNode(df), arr_h1_phi, "h1_spanet_boosted_phi_v34bis")
df = ROOT.AddBoolArray(ROOT.RDF.AsRNode(df), arr_h1_match, "h1_spanet_boosted_match_v34bis")

df = ROOT.AddArray(ROOT.RDF.AsRNode(df), arr_h2_mass, "h2_spanet_boosted_mass_v34bis")
df = ROOT.AddArray(ROOT.RDF.AsRNode(df), arr_h2_pt, "h2_spanet_boosted_pt_v34bis")
df = ROOT.AddArray(ROOT.RDF.AsRNode(df), arr_h2_eta, "h2_spanet_boosted_eta_v34bis")
df = ROOT.AddArray(ROOT.RDF.AsRNode(df), arr_h2_phi, "h2_spanet_boosted_phi_v34bis")
df = ROOT.AddBoolArray(ROOT.RDF.AsRNode(df), arr_h2_match, "h2_spanet_boosted_match_v34bis")

df = ROOT.AddArray(ROOT.RDF.AsRNode(df), arr_h3_mass, "h3_spanet_boosted_mass_v34bis")
df = ROOT.AddArray(ROOT.RDF.AsRNode(df), arr_h3_pt, "h3_spanet_boosted_pt_v34bis")
df = ROOT.AddArray(ROOT.RDF.AsRNode(df), arr_h3_eta, "h3_spanet_boosted_eta_v34bis")
df = ROOT.AddArray(ROOT.RDF.AsRNode(df), arr_h3_phi, "h3_spanet_boosted_phi_v34bis")
df = ROOT.AddBoolArray(ROOT.RDF.AsRNode(df), arr_h3_match, "h3_spanet_boosted_match_v34bis")

df = ROOT.AddArray(ROOT.RDF.AsRNode(df), arr_prob_hhh, "ProbHHH_v34bis")
df = ROOT.AddArray(ROOT.RDF.AsRNode(df), arr_prob_qcd, "ProbQCD_v34bis")
df = ROOT.AddArray(ROOT.RDF.AsRNode(df), arr_prob_tt, "ProbTT_v34bis")
df = ROOT.AddArray(ROOT.RDF.AsRNode(df), arr_prob_hh4b, "ProbHH4b_v34bis")
#df = ROOT.AddArray(ROOT.RDF.AsRNode(df), arr_prob_vjets, "ProbVJets_v34bis")
#df = ROOT.AddArray(ROOT.RDF.AsRNode(df), arr_prob_vv, "ProbVV_v36")
#df = ROOT.AddArray(ROOT.RDF.AsRNode(df), arr_prob_hhh4b2tau, "ProbHHH4b2tau_v36")
#df = ROOT.AddArray(ROOT.RDF.AsRNode(df), arr_prob_hh4b, "ProbHH4b_v36")
#df = ROOT.AddArray(ROOT.RDF.AsRNode(df), arr_prob_hh2b2tau, "ProbHH2b2tau_v36")
#df = ROOT.AddArray(ROOT.RDF.AsRNode(df), arr_prob_dy, "ProbDY_v35")
