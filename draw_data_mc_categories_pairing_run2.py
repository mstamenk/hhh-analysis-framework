...
# variables = ROOT.std.vector['string'](["h_fit_mass","h1_t3_mass","h2_t3_mass","h3_t3_mass"])
# variables = ROOT.std.vector['string'](["ProbMultiH"])
variables = ROOT.std.vector['string'](["ProbMultiH","h1_mass","h2_mass","h3_mass","h_mass","h1_spanet_boosted_mass","h2_spanet_boosted_mass","h3_spanet_boosted_mass"])
...
for var in variables:
    ...
    if do_log :
        plot_file = "%s/AN_%s_log" % (output_folder,str(var))
        if save_pdf :
        ...
        plot_file = "%s/AN_%s" % (output_folder,str(var))
        if save_pdf :
            canvas.Print("%s%s" % (plot_file, '.pdf') ) # save PDF only when we need to add to docs
        canvas.Print("%s%s" % (plot_file, '.png'))
