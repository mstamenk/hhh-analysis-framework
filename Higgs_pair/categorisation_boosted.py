...
for wp in working_points:
    ...
    df_2d.Draw("COLZ")
    df_2d.GetXaxis().SetTitle("true Category")  # X轴标题
    ...
    for i in range(0, 10):  # 遍历 X 轴的所有值
        ...
        total_count_reco_x = df.Filter(f"categorisation == {i}").Count().GetValue()
    ...
    canvas.Update()
    canvas.SaveAs("output_2d_%s_boosted_prio.pdf"%wp)
