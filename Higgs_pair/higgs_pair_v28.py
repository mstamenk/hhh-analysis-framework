...
cat_list = ["ProbHHH6b_0bh3h_inclusive_"]
# file_list = ["data_obs","QCD","GluGluToHHHTo6B_SM","GluGluToHHTo4B_cHHH1"]
file_list = ["data_obs"]
for cat in cat_list:
    for file in file_list:
        input_file = ROOT.TFile.Open("%s/%s/%s.root"%(path,cat,file), "READ")
        tree = input_file.Get("Events") 
        print ("processing: %s/%s/%s"%(path,cat,file)) # 替换为实际的树名

        # 创建输出文件并复制树结构
        output_file = ROOT.TFile("%s/%s/%s_new.root"%(path,cat,file), "RECREATE")
        ...
        for entry_num, event in enumerate(tree):
            ...
            jets_4vec = []
            for i in range(1, 7):  # 假设有 10 个 jet
