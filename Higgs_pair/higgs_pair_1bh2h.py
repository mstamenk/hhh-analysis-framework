...
# cat_list = ["ProbHHH6b_3bh0h_inclusive_CR","ProbHHH6b_2bh1h_inclusive_CR","ProbHHH6b_1bh2h_inclusive_CR","ProbHHH6b_1bh2h_inclusive_CR"]
cat_list = ["ProbHHH6b_1bh2h_inclusive_CR"]
file_list = ["data_obs","QCD_datadriven","GluGluToHHHTo6B_SM","GluGluToHHTo4B_cHHH1"]
for year in year_list:
    for cat in cat_list:
        for file in file_list:
            input_file = ROOT.TFile.Open("%s/%s/%s/%s.root"%(path,year,cat,file), "READ")
            tree = input_file.Get("Events") 
            print ("processing: %s/%s/%s/%s"%(path,year,cat,file)) # 替换为实际的树名

            # 创建输出文件并复制树结构
            output_file = ROOT.TFile("%s/%s/%s/%s_new.root"%(path,year,cat,file), "RECREATE")
